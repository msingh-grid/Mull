...
| ~~⌥Space handled by two listeners~~ | closed in M3 — a sidecar `CGEventTap` both observes and swallows; the old pair remains as a fallback | done |
| ~~HUD is an unstyled read-out~~ | closed in M3 — the Studio Paper panel, see `docs/M3-VERIFY.md` | done |
