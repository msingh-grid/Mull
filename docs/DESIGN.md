...
3. *Four permissions, each with a reason* — Microphone / Accessibility / Input Monitoring / Screen Recording as plan-style step rows (·/…/✓ state cells per §6.4), each stating its reason; each Grant button deep-links System Settings and the ✓ comes from live permission polling, never from the click. Screen Recording is marked **optional** and its button says *Grant, then relaunch*: Mull dictates, edits and composes without it and simply never sees a picture, and macOS caches that grant per process so no ✓ can appear until the app is launched again. Its Grant also calls `CGRequestScreenCaptureAccess` before opening the pane — an app that has never requested screen capture is not listed there at all. Secure-input pause disclosed up front via a warn chip.
